...
df = pd.read_csv("toris_atlas.csv", encoding="ISO-8859-1", engine="python")

dfcommercial = pd.read_csv("commercial.csv", encoding="ISO-8859-1", engine="python")
...
plt.grid(False)
plt.savefig(
    "TORIS_AtlasXGBC_average_impact_on_model_output_magnitude.png",
    bbox_inches="tight",
    facecolor="w",
    dpi=1200,
)
...
plt.grid(False)
plt.savefig(
    "TORIS_AtlasXGBR_impact_on_model_output.png",
    bbox_inches="tight",
    facecolor="w",
    dpi=1200,
)
...
dfcommercial = dfcommercial[
    (dfcommercial["RECOVERY FACTOR"] >= 0) | (dfcommercial["RECOVERY FACTOR"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["RECOVERY FACTOR"] <= 1) | (dfcommercial["RECOVERY FACTOR"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["POROSITY"] >= 0) | (dfcommercial["POROSITY"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["POROSITY"] <= 1) | (dfcommercial["POROSITY"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["WATER SATURATION"] >= 0)
    | (dfcommercial["WATER SATURATION"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["WATER SATURATION"] <= 1)
    | (dfcommercial["WATER SATURATION"].isnull())
]
...
dfcommercial = dfcommercial[
    (dfcommercial["FVF"] <= 10) | (dfcommercial["FVF"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["GOR"] >= 0) | (dfcommercial["GOR)"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["GOR"] <= 60) | (dfcommercial["GOR"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["RESERVES"] >= 0) | (dfcommercial["RESERVES"].isnull())
]

dfcommercial = dfcommercial[
    (dfcommercial["RESERVES"] <= 5e11) | (dfcommercial["RESERVES"].isnull())
]
...
commercial_score = commercial_accu.append(
    {"Train": "Train", "accuracy_value": scorecommercial}, ignore_index=True
)
...
y_predcommercial_values = pd.concat(
    [
        y_predcommercial_values,
        pd.DataFrame({"y_measured": ycommercial, "y_estimated": y_predcommercial}),
    ],
    ignore_index=True,
)
