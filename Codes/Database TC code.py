...
df = pd.read_csv("toris_commercial.csv", encoding="ISO-8859-1", engine="python")

dfatlas = pd.read_csv("Atlas.csv", encoding="ISO-8859-1", engine="python")
...
plt.grid(False)
plt.savefig(
    "TORIS_commercialXGBC_average_impact_on_model_output_magnitude.png",
    bbox_inches="tight",
    facecolor="w",
    dpi=1200,
)
...
plt.grid(False)
plt.savefig(
    "TORIS_commercialXGBR_impact_on_model_output.png",
    bbox_inches="tight",
    facecolor="w",
)
...
dfatlas = dfatlas[
    (dfatlas["RECOVERY FACTOR"] >= 0) | (dfatlas["RECOVERY FACTOR"].isnull())
]

dfatlas = dfatlas[
    (dfatlas["RECOVERY FACTOR"] <= 1) | (dfatlas["RECOVERY FACTOR"].isnull())
]
...
dfatlas = dfatlas[
    (dfatlas["WATER SATURATION"] >= 0) | (dfatlas["WATER SATURATION"].isnull())
]

dfatlas = dfatlas[
    (dfatlas["WATER SATURATION"] <= 1) | (dfatlas["WATER SATURATION"].isnull())
]
...
dfatlas = dfatlas[(dfatlas["RESERVES"] <= 5e11) | (dfatlas["RESERVES"].isnull())]
...
atlas_score = atlas_accu.append(
    {"Train": "Train", "accuracy_value": scoreatlas}, ignore_index=True
)
...
y_predatlas_values = pd.concat(
    [
        y_predatlas_values,
        pd.DataFrame({"y_measured": yatlas, "y_estimated": y_predatlas}),
    ],
    ignore_index=True,
)
