...
plt.grid(False)
plt.savefig(
    "commercial_AtlasXGBC_average_impact_on_model_output_magnitude.png",
    bbox_inches="tight",
    facecolor="w",
    dpi=1200,
)
...
plt.grid(False)
plt.savefig(
    "commercial_AtlasXGBR_impact_on_model_output.png",
    bbox_inches="tight",
    facecolor="w",
    dpi=1200,
)
...
dftoris = dftoris[
    (dftoris["RECOVERY FACTOR"] >= 0) | (dftoris["RECOVERY FACTOR"].isnull())
]

dftoris = dftoris[
    (dftoris["RECOVERY FACTOR"] <= 1) | (dftoris["RECOVERY FACTOR"].isnull())
]
...
dftoris = dftoris[
    (dftoris["WATER SATURATION"] >= 0) | (dftoris["WATER SATURATION"].isnull())
]

dftoris = dftoris[
    (dftoris["WATER SATURATION"] <= 1) | (dftoris["WATER SATURATION"].isnull())
]
...
dftoris = dftoris[(dftoris["RESERVES"] <= 5e11) | (dftoris["RESERVES"].isnull())]
...
toris_score = toris_accu.append(
    {"Train": "Train", "accuracy_value": scoretoris}, ignore_index=True
)
...
y_predtoris_values = pd.concat(
    [
        y_predtoris_values,
        pd.DataFrame({"y_measured": ytoris, "y_estimated": y_predtoris}),
    ],
    ignore_index=True,
)
